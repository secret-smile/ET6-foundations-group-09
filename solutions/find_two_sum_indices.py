...
def find_two_sum_indices(nums: list[float], target: float) -> list[int]:
    ...
    assert all(isinstance(num, (int, float)) for num in nums), "List must contain numbers (integers or floats)."
    assert isinstance(target, (int, float)), "Target must be a number (integer or float)."
